...
// doc: 每个相机都有一个FeatureTracker实例，即trackerData[i]
...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    // doc: 通过时间间隔判断相机数据流是否稳定，有问题则restart
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)    // doc: 单目时 i = 0
        {
            ...
            for (unsigned int j = 0; j < ids.size(); j++) // doc: 特征点数量
            {
                if (trackerData[i].track_cnt[j] > 1)
                {
                    ...
                    id_of_point.values.push_back(p_id * NUM_OF_CAM + i); // doc: 单目相当于 push_back(p_id)
        ...
        if (SHOW_TRACK)
        {
            ...
            for (int i = 0; i < NUM_OF_CAM; i++)
            {
                ...
                // doc: 显示追踪状态，越红越好，越蓝越不行
                for (unsigned int j = 0; j < trackerData[i].cur_pts.size(); j++)
                {
                    // doc: 根据追踪的次数来衡量特征点的质量，越红越好，越蓝越不行
...
int main(int argc, char **argv)
{
    ...
    // doc: 订阅话题IMAGE_TOPIC(/cam0/image_raw),执行回调函数img_callback
    ros::Subscriber sub_img = n.subscribe(IMAGE_TOPIC, 100, img_callback);

    // doc: 发布feature，实例feature_points，跟踪的特征点，给后端优化用
    pub_img = n.advertise<sensor_msgs::PointCloud>("feature", 1000);
    // doc: 发布feature_img，实例ptr，跟踪的特征点图，给RVIZ用和调试用
