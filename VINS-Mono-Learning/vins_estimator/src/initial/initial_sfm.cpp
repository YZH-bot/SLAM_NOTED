			  ...
			  vector<SFMFeature> &sfm_f, map<int, Vector3d> &sfm_tracked_points)
{
	...
	// doc: 1、先三角化第l帧（参考帧）与第frame_num-1帧（当前帧）之间的路标点
	// doc: 2、pnp求解从第l+1开始的每一帧到第l帧的变换矩阵R_initial, P_initial，保存在Pose中
	// doc: 并与当前帧进行三角化
	for (int i = l; i < frame_num - 1 ; i++)
	{
		...
		// doc: 因为 if (i > l), 会先三角化参考帧和当前帧
		triangulateTwoFrames(i, Pose[i], frame_num - 1, Pose[frame_num - 1], sfm_f);
	}

	// doc: 3、对第l帧与第l+1到frame_num -2的每一帧再进行一次三角化, 之前都是相对当前帧的三角化
	for (int i = l + 1; i < frame_num - 1; i++)
		triangulateTwoFrames(l, Pose[l], i, Pose[i], sfm_f);


	// doc: 4、PNP求解从第l-1到第0帧的每一帧与第l帧之间的变换矩阵，并进行三角化
	for (int i = l - 1; i >= 0; i--)
	{
		//solve pnp
		Matrix3d R_initial = c_Rotation[i + 1];
	...
	// doc: 5、三角化其他未恢复的特征点。
	// doc: 至此得到了滑动窗口中所有图像帧的位姿以及特征点的3d坐标
	for (int j = 0; j < feature_num; j++)
	{
		if (sfm_f[j].state == true)
			continue;
