...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td)
{
    ...
    for (auto &id_pts : image)
    {
        ...
        else if (it->feature_id == feature_id)
        {
            ...
            // doc: last_track_num，表示此帧有多少个和其他帧中相同的特征点能够被追踪到, 用于近似共视
            last_track_num++;
        }
    }

    // doc: 如果关键帧数量太少或者与其他帧共视太少则视为关键帧
    if (frame_count < 2 || last_track_num < 20)
        return true;

    // doc: 计算每个特征在次新帧和次次新帧中的视差
    for (auto &it_per_id : feature)
    {
        // doc: 计算能被当前帧和其前两帧共同看到的特征点视差        
        if (it_per_id.start_frame <= frame_count - 2 && // doc: 次次新帧 和 次新帧
            it_per_id.start_frame + int(it_per_id.feature_per_frame.size()) - 1 >= frame_count - 1) // doc: 必须包含当前帧
        {
            // doc: 计算特征点it_per_id在倒数第二帧和倒数第三帧之间的视差，并求所有视差的累加和
            // doc: 计算视差参考链接：https://blog.csdn.net/weixin_42846216/article/details/106634318
...
double FeatureManager::compensatedParallax2(const FeaturePerId &it_per_id, int frame_count)
{
    ...
    // doc: 因为特征点都是归一化之后的点，所以深度都为1，这里没有去除深度，下边去除深度，效果一样。
    double u_j = p_j(0);
    double v_j = p_j(1);
    ...
    // doc: 这一步与上一步重复，不知道必要性在哪里，目前没有必须性
    double dep_i_comp = p_i_comp(2);
    double u_i_comp = p_i_comp(0) / dep_i_comp;
    double v_i_comp = p_i_comp(1) / dep_i_comp;
    double du_comp = u_i_comp - u_j, dv_comp = v_i_comp - v_j;

    // doc: 视差距离计算。难道这里min中的两个平方距离的计算值不一样吗？？？
    // doc: 而且sqrt为非负实数，肯定是要大于零的，为什么还要和零做比较？
