...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    {
        TicToc tmp_t;
        // doc: 通过词袋DBoW的方式进行回环检测，返回回环候选帧的索引
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        // doc: 闭环检测成功, optimize_buf不为空, 取出最新一个待优化帧作为当前帧
        m_optimize_buf.lock();
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                // doc: 固定相邻帧add edge, 添加序列边, 即关键帧数据库中关键帧两两之间的相对位姿约束
                for (int j = 1; j < 5; j++)
                {
