...
#pragma omp parallel for num_threads(num_threads_) firstprivate(k_indices, k_sq_dists) schedule(guided, 8)
  for (int i = 0; i < input_->size(); i++) {
    ...
    sq_distances_[i] = k_sq_dists[0]; // info：记录最小平方距离值
    correspondences_[i] = k_sq_dists[0] < corr_dist_threshold_ * corr_dist_threshold_ ? k_indices[0] : -1;  // info：判断是否小于阈值，是的话存储该点在target中的index
