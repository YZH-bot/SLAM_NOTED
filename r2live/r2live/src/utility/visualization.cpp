...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_path = n.advertise<nav_msgs::Path>("path", 1000);  //; 最后发的话题应该是r2live/path
...
void pub_LiDAR_Odometry(const Estimator &estimator, const StatesGroup &state, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        Quaterniond tmp_Q;
        tmp_Q = Quaterniond(g_lio_state.rot_end);
        ...
        pub_odometry.publish(odometry);   //; 发送里程计，用的是滤波结果的位姿

        if (g_camera_lidar_queue.m_if_write_res_to_bag)
        {
            g_camera_lidar_queue.m_bag_for_record.write(pub_odometry.getTopic(), odometry.header.stamp, odometry);
        }
        
        //; 重要：发布轨迹path，用的也是滤波的结果
        ...
        pub_path.publish(path);  //; 发布轨迹，用的也是滤波结果的位姿

        if (g_camera_lidar_queue.m_if_write_res_to_bag)
        {
            g_camera_lidar_queue.m_bag_for_record.write(pub_path.getTopic(), pose_stamped.header.stamp, path);
        }
...
void pubCameraPose(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        pub_camera_pose.publish(odometry);  //; 发布话题 camera_pose
...
void pub_LiDAR_pose(const Estimator &estimator, const std_msgs::Header &header, eigen_q extrinsic_q, vec_3 extrinsic_t)
{
    int idx2 = WINDOW_SIZE - 1;
    ...
    {
        int i = idx2;
        Vector3d P = estimator.Ps[i] + extrinsic_t + estimator.Rs[i] * estimator.tic[0];
        Quaterniond R = Quaterniond(estimator.Rs[i] * extrinsic_q.toRotationMatrix() * estimator.ric[0]);
