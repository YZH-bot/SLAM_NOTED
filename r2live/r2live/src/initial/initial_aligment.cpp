...
/*
    重力矢量细化
    重力细化，在其切线空间上用两个变量重新参数化重力 -> Paper V-B-3
    g^ = ||g|| * (g^-) + w1b1 + w2b2
...
{
    // g0 = (g^-)*||g||
    Vector3d g0 = g.normalized() * G_gravity.norm();
    Vector3d lx, ly;
    ...
    for (int k = 0; k < 4; k++) // 迭代4次
    {
        ...
        g0 = (g0 + lxly * dg).normalized() * G_gravity.norm();
    }
...
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    if (fabs(g.norm() - G_gravity.norm()) > 1.0 || s < 0)
    {
        return false;
    }
