...
struct orgtype
{
  ...
  double dista;     // 当前点与后一个点距离的平方
  double angle[2];  // 当前点与前一个点夹角、当前点与后一个点夹角
  double intersect; // 前一点和后一点夹角
...
int main(int argc, char **argv)
{
  ...
  // 从launch读取配置参数
  n.param<int>("feature_extraction/lidar_type", lidar_type, 0);                // 雷达类型，包含MID40、HORIZON、VELO16、OUST64
  ...
  //; 配置文件中给的是lidar_type=1, 也就HORIZON类型
  case HORIZON:
    printf("HORIZON\n");
...
void mid_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  pcl::fromROSMsg(*msg, pl); // 点云转换为pcl格式
  ...
  give_feature(pl, types, pl_corn, pl_surf); // 提取特征点
...
//; r2live默认运行的就是livox horizon类型的lidar。所以这里使用的是livox的驱动中自定义的消息类型CustomMsg
void horizon_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  double t1 = omp_get_wtime();
  vector<pcl::PointCloud<PointType>> pl_buff(N_SCANS);   //; N_SCANS是6
  ...
  {
    if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10) && (!IS_VALID(msg->points[i].x)) && (!IS_VALID(msg->points[i].y)) && (!IS_VALID(msg->points[i].z)))
    {
      ...
      if ((std::abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7) || (std::abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7) || (std::abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7))
      {
  ...
  //; 提取出激光雷达的角点、面点和所有点，并发布
  pub_func(pl_full, pub_full, msg->header.stamp);
  pub_func(pl_surf, pub_surf, msg->header.stamp);  //; 被fast_lio使用的平面点
  pub_func(pl_corn, pub_corn, msg->header.stamp);
