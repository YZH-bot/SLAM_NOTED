...
template <typename T = double>
T cot(const T theta) // 余切 tan × cot = 1
{
    return 1.0 / std::tan(theta);
}

template <typename T = double>
// 通过罗德里格斯方程将旋转向量omega转旋转矩阵
...
struct Camera_Lidar_queue
{
    ...
    int m_if_acc_mul_G = 0;  //; 加速度值是否以重力加速度g为模长的标志

    int m_if_have_lidar_data = 0;
    int m_if_have_camera_data = 0;
    int m_if_lidar_can_start = 1;   //; 这个是什么标志？
    ...
    double get_lidar_front_time()
    {
        ...
        {
            return m_liar_frame_buf->front()->header.stamp.toSec() + 0.1;  //; 为什么+0.1？
        }
        else
        {
            return -3e8;
    ...
    bool if_camera_can_process()
    {
        ...
        //; 最老的lidar时间比camera时间早，那么要先处理lidar数据
...
struct StatesGroup
{
    ...
    // 传入状态的增量，更新状态值。增量顺序分别是：旋转、平移、速度、bias_g、bias_a
...
{
    //; Pose6D是一个自定义的ros msg，定义了IMU预积分相对于lidar起始位姿的PVQ和acc gyro
    Pose6D rot_kp;
    rot_kp.offset_time = t;  //; 相对lidar起始位姿的偏移时间
    for (int i = 0; i < 3; i++)
    {
        rot_kp.acc[i] = a(i);
        rot_kp.gyr[i] = g(i);
