...
void ROS1Visualizer::setup_subscribers(std::shared_ptr<ov_core::YamlParser> parser) {
  ...
  if (_app->get_params().state_options.num_cameras == 2) {
    ...
    // doc: Now we should add any non-stereo callbacks here
...
void ROS1Visualizer::callback_inertial(const sensor_msgs::Imu::ConstPtr &msg) {
  ...
  // doc: send it to our VIO system
  _app->feed_measurement_imu(message);  // doc: VIO system
  ...
  std::thread thread([&] {
    // doc: Lock on the queue (prevents new images from appending)
    std::lock_guard<std::mutex> lck(camera_queue_mtx);

    // doc: Count how many unique image streams
    std::map<int, bool> unique_cam_ids;
    for (const auto &cam_msg : camera_queue) {
      unique_cam_ids[cam_msg.sensor_ids.at(0)] = true;
    }

    // doc: If we do not have enough unique cameras then we need to wait
    // doc: We should wait till we have one of each camera to ensure we propagate in the correct order
...
void ROS1Visualizer::callback_monocular(const sensor_msgs::ImageConstPtr &msg0, int cam_id0) {
  ...
  // doc: append it to our queue of images
  std::lock_guard<std::mutex> lck(camera_queue_mtx);
