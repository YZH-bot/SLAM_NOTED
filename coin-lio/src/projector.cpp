...
void Projector::createImages(LidarFrame& frame) const {
    ...
    frame.proj_idx = std::vector<int>(rows_*cols_*DUPLICATE_POINTS, 0); // doc: DUPLICATE_POINTS ???
    ...
    for (size_t j = 0; j < frame.points_corrected->points.size(); ++j) {
        ...
        // doc: 根据原始未去畸变前的index去索引，因此图像都是未去畸变前的，因此求雅可比的时候引入了畸变变换 参见issue9：https://github.com/ethz-asl/COIN-LIO/issues/9
