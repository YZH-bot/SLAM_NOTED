...
/**
 * doc:getSubPixelValue，用于从图像中获取浮点坐标位置附近的子像素值。
 * 在函数中，首先对输入的坐标位置 (x, y) 进行边界限定，确保它们不超过图像的尺寸范围。然后将输入的浮点坐标位置 (x, y)
 * 转换为其所在整数坐标的下限 (x_lower, y_lower) 和上限 (x_upper, y_upper)。随后，计算出 (x, y) 相对于 (x_lower, y_lower) 
 * 左下角像素的相对偏移量 xx 和 yy。最后，利用双线性插值的方法，根据相对偏移量 xx 和 yy 在四个最近的像素值上进行加权求和，
 * 得到最终的子像素值，并将其返回。
*/
template<typename T>
